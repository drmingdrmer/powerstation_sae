...
class template {
	...
	function parse_template($tplfile, $templateid, $tpldir, $file, $cachefile) {
		...
		if(!@$fp = fopen('saemc://'.$cachefile, 'w')) {
			$this->error('directory_notfound', dirname('saemc://'.$cachefile));
		}
		...
		$template = str_replace('href="data/cache/','href="http://'.$_SERVER['HTTP_APPNAME'].'-discuzx.stor.sinaapp.com/data/cache/',$template);
		$template = str_replace('href=\'data/cache/','href=\'http://'.$_SERVER['HTTP_APPNAME'].'-discuzx.stor.sinaapp.com/data/cache/',$template);
		$template = str_replace('src="data/cache/','src="http://'.$_SERVER['HTTP_APPNAME'].'-discuzx.stor.sinaapp.com/data/cache/',$template);
		$template = str_replace('src=\'data/cache/','src=\'http://'.$_SERVER['HTTP_APPNAME'].'-discuzx.stor.sinaapp.com/data/cache/',$template);
	...
	function loadcsstemplate() {
		...
		$s = new SaeStorage();
 		$content = $s->read( 'discuzx' , 'data/cache/style_'.STYLEID.'_module.css');
		...
		if($this->csscurmodules) {
			...
			$s->write( 'discuzx' , 'data/cache/style_'.STYLEID.'_'.$_G['basescript'].'_'.CURMODULE.'.css',$this->csscurmodules);
