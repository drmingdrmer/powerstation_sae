...
function stream_save($strdata, $albumid = 0, $fileext = 'jpg', $name='', $title='', $delsize=0, $from = false) {
	...
	$newfilename = str_replace(array(DISCUZ_ROOT,'./','//'),array('','','/'),$newfilename);
	$s = new SaeStorage();
			 
 	if($filepath = $s->write('discuzx',$newfilename,$strdata) ){
 		$attrs = $s->getAttr('discuzx',$newfilename);
		$size = $attrs['length'];
