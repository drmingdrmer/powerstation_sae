...
Class discuz_upload{
	...
	function save($ignore = 0) {		
		...
		if(!$file_url = $this->save_to_local($this->attach['tmp_name'], $this->attach['target'])) {
			$this->errorcode = -103;
			return false;
		} else {
			$this->errorcode = 0;
	...
	function get_image_info($target, $allowswf = false) {
		$s = new SaeStorage();
		$img_data = $s->read('discuzx',$target);
		if($img_data)
		{
			$img = new SaeImage();
			$img->setData( $img_data );
	 		return $img->getImageAttr();
	 	}
	 	else
	 	{
	...
	function save_to_local($source, $target) {
		...
		{
			$s = new SaeStorage();			
		 	$destFile = str_replace(array(DISCUZ_ROOT,'./','//'),array('','','/'),$target);
		 
		 	if($attach_url = $s->upload('discuzx',$destFile,$source) ){
		 		$succeed = true;
		 		$this->attach['attachment'] = $attach_url;
		 	}
		 	else
		 	{
		 		$succeed = false;
		...
		$this->attach['target'] = str_replace(array(DISCUZ_ROOT,'./','//'),array('','','/'),$this->attach['target']);
		$s = new SaeStorage();
		$s->write( 'discuzx' , 'log.txt' , DISCUZ_ROOT.'----'.var_export($this->attach,true));
		$this->errorcode = 0;
		return $attach_url;
	}
